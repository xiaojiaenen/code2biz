...
**迁移表必须业务化（R12，详见 `references/13-business-deep-reading.md`）**：交付单位是"业务动作"，不是状态字面量。每条迁移在四列之外必须补三列：**触发者**（谁/哪个模块/哪类定时器，用 codegraph `callers` 回溯）、**业务动作**（`biz_action`，如任务创建/资源占用/执行下发/完成回执，词表见 13 号文档）、**业务意义**（`biz_meaning`，这个流转完成后业务世界里什么变了）。每个状态同时要有 `biz_definition`（业务含义：谁让它进入、离开意味着什么），落到 `schemas/phase1.schema.json` 的 `states[].biz_definition` 与 `transitions[].actor/biz_action/biz_meaning` 字段。

**实测教训（Russh，必须避免重蹈）**：6 台状态机的交付形态是 `unknown → running` 这种字面量流转，15 处触发条件写成"（同上触发，未单列）"——枚举字面量照抄进文档是技术噪声，读者获得的信息量为零。两个硬要求：

- **`unknown` 这类"看起来像解析失败"的状态恰恰最需要解释**：它是"记录先于调度存在"（已受理未开工）、迁移兜底、还是废弃残留？三种成因业务含义完全不同。解释不了标 `needs_confirmation=true` 进待确认清单，**禁止照抄字面量充数**。
- **状态的业务定义要回答"此刻业务世界里什么是真的"**（资源已占用/款已到账/库存已锁定），不是复述状态名（"running 就是运行中"）。
...
- `recorded`：业务动作单一（典型是 CRUD / 单表查询），但**它仍然是真实业务**——必须展开成"输入 / 输出 / 业务目的 / 失败边界"四要素，在文档里有 ≥60 可见字符的独立说明。**不是"一行归类说明"**
- `flagged`：识别为异常/特例入口，进 Phase 5 待确认清单

**关于 `recorded`，这个定义是被实测改过的**：它原先写的是"业务逻辑单薄，在文档里给一行归类说明即可，不需要展开"。Russh 实测的结果是 38 个入口点里 29 个被标成 `recorded`（76.3%），文档对这 29 条的全部交代是架构章节里的一句"其余业务 API 见数据层说明"。**一句话带过不是"简要"，是没讲**——而且比不写更有害，因为它让读者以为文档已经覆盖了。

所以 `recorded` 现在有硬门槛：四要素齐全 + ≥60 可见字符 + **占已处理入口点的比例不得超过 50%**。占比超标的含义通常是"分类粒度太粗"——把几个本该 `detailed` 的流程合并成了 `recorded`，或者根本没看进去就标了。CRUD 密集的管理后台应该**按资源分组**展开（"节点管理这组接口共同维护主机台账，其中 DELETE 是软删…"），而不是逐条标 recorded 再一句话带过。模板见 `references/12-content-depth-gate.md` §3，由 `scripts/check-content-depth.mjs`（R9-4）机器检查。
...
**要正确理解那个百分比**：Russh 实测产物的覆盖率是 100.0%，但它的实际含义只是"38 条每条都打了状态标记"，不代表"38 条都讲清楚了"——其中 29 条是 `recorded`，文档对它们的交代只有一句话。**分子是"有状态"，不是"讲清楚了"**。后者由 `scripts/check-content-depth.mjs` 的 R9-4 单独把关（recorded 占比上限 + 每条业务说明字数下限 + 逐条核对是否真在文档里出现）。覆盖率达标之后还要跑内容深度门禁，两个都过才算完成 R8。
